...
_default_tfmer_cfg = dict(d_model=512, nhead=8, d_inner=2048, # 1024
                          dropout=0.1, activation='relu',dataset_max_length=25)
import copy
import math
import warnings
...
class Model(nn.Module):
    ...
    def __init__(self, config=_default_tfmer_cfg):
        super().__init__()
        # print("??",config)
        self.max_length = config['dataset_max_length'] + 1
        self.charset = CharsetMapper('./model/charset.txt', max_length=self.max_length)
    ...
    def _get_length(self, logit, dim=-1):
        ...
        out = (logit.argmax(dim=-1) == 0)#self.charset.null_label)
        abn = out.any(dim)
...
class BCNLanguage(Model):
    def __init__(self, config=_default_tfmer_cfg):
        ...
        self.proj = nn.Linear(37, d_model, False)
    ...
    def forward(self, tokens, lengths):
        ...
        Args:
            ...
            lengths: (N,)
        """
        # print(tokens.size(),lengths.size(),lengths.sum())
        # txt=self.charset.get_text(tokens[0].argmax(dim=1), padding=False, trim=False)
        # print(txt,':::')
        ...
        # txt=self.charset.get_text(logits[0].argmax(dim=1), padding=False, trim=False)
        # print(txt,':::')
        res =  {'feature': output, 'logits': logits,
...
m.eval()
converter = m.charset
encode = converter.get_labels("beautifds")
encode = np.array(encode)
# print(encode)
one_hot = F.one_hot(torch.from_numpy(encode),num_classes = 37)
# add dim
one_hot = one_hot.unsqueeze(0)
print(one_hot.size())
# txt = converter.get_text(encode)
l = m._get_length(one_hot)
# print("L=",l,'vs',len('beautifds'))
# x = torch.randn((2,26,37))
# l = torch.IntTensor([26])
print(type(one_hot),type(l))
res = m(one_hot.float(),l)
print(res['logits'].size())
logits = res['logits']
logits = F.softmax(logits,2)
print(logits.size())
text = converter.get_text(logits[0].argmax(dim=1), padding=False, trim=False)
print(text)